...
S_3_TT <- TT(I_94_EB_TT_melt,225.63,228.52,'15 mins')
...
```{r S_3_MNCMAT}
S_3_MNCMAT <- MNCMAT_final %>% filter(Measure_Round>=225.63 &Measure_Round<=228.52 & Direction =='94W') %>% as.data.frame()
...
```{r S_3_IRIS}
S_3_IRIS <- IRIS_2015_event %>% filter(Measure_Round >=225.63 &Measure_Round<=228.52 & Direction == ' WB')
...
```{r S_3_final}
S_3_final <- S_3_merge %>% select(DateTime,VMT_total,TT_mean_to,weather,Crash,INCIDENT,ROADWORK)
S_3_final$VMT_total[S_3_final$VMT_total == 0] <- NA
S_3_final$VMT_total[S_3_final$VMT_total < 0] <- NA
write.csv(S_3_final,'S_3_final_rev2.csv',na='Null',row.names = FALSE)
```
